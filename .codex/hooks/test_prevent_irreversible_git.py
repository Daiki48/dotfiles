...
class HookTest(unittest.TestCase):
    ...
    def test_blocks_irreversible_operations(self):
        for command in (
            ...
            "bash -lc 'rm important.txt'",
            "sh -ec 'git push --force origin feature/example'",
            "sh -c 'git push --force origin feature/example'",
            "nice rm important.txt",
            "timeout 1 rm important.txt",
            "timeout -s KILL 1 rm important.txt",
            "exec -a safe-name rm important.txt",
            ...
            "git worktree repair /tmp/worktree",
            "git worktree unlock /tmp/worktree",
            "git worktree add --force /tmp/worktree feature/example",
