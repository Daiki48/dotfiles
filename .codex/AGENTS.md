...
- 必要な読み取り調査の後、最初の編集前に依頼の目的、受け入れ条件、非目標を再確認する。依頼に明記されていない作業を検討したときは、その着手前に元の目的へ立ち返り、目的達成または安全な検証に必要なら進め、単に望ましい改善なら見送る。必要性を確定できず実質的にスコープが広がる場合だけDaikiへ確認し、受け入れ条件と必須検証を満たした時点で終了する。
