...
- `finish`が変更対象の`.codex/`または`.agents/`に対するruntimeのread-only拒否を識別し、再試行tokenを発行した場合だけ、同じtask・PR・head・planを指定した`codex-delivery finish --sandbox-retry`をsandbox外権限で実行します。helperがtokenを実行前に消費するため再試行は1回に限られます。token不在や再失敗では権限昇格せず、直接`git merge`やcleanupへ迂回しません。
