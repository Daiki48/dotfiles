...
- 親agentは問題定義を含む日常的な調査・計画・実装に`gpt-5.6-terra`のmediumを使い、Plan modeだけhighにする
- subagentの既定値は`gpt-5.6-terra`のmediumとし、独立して並行できる作業だけを委譲する
- 明確で狭い検索、ログ整理、機械的比較には`gpt-5.6-luna`のmedium、仕様が固定された狭い実装には同modelのxhighを明示する
- 複雑な実装・互換性reviewには`gpt-5.6-terra`のmediumまたはhighを使う
- `gpt-5.6-sol`のhighは曖昧性が残る問題定義、重大なセキュリティreview、難しい設計判断、他modelの結論が衝突した場合に限定する
...
4. GitHub repositoryでは承認済み計画を追跡Issueへ記録する。GitHubを使えない場合だけproject docsまたは同一sessionの合意を正本にする
5. 計画の正本と実行許可を確認したら、作業ブランチ上で全実装単位を連続して実装・検証する
...
- 正本は、信頼済み追跡Issue、プロジェクト指定docs、同一セッションの明示的合意の順で特定する
