...
- rules、hook、sandbox、auto-reviewによる拒否だけを理由に作業を終了したり、final応答へ移行したりしない。拒否理由を確認し、まず許可済みの単一・直接commandへ引数や前提を正規化する。
- 同じ結果を得る迂回、権限拡張、`danger-full-access`、拒否された操作の反復は行わない。不要な操作を省略し、目的を保つより安全な非破壊経路へ切り替え、独立して進められる残作業を完了する。
- auto-reviewのtimeoutは危険操作の許可・拒否と同一視しない。外部状態を再確認し、同一操作を繰り返さない安全な再開条件がある場合だけ続行する。
- 許可済み経路では必須成果を達成できず、安全な代替もない場合に限り、拒否理由、完了済み作業、必要な最小承認を示してDaikiへ確認する。
