...
- `finish`が変更対象の`.codex/`または`.agents/`に対するpermission拒否を識別し、再試行tokenを発行した場合だけ、同じtask・PR・head・planを指定した`codex-delivery finish --sandbox-retry`を同一UIDのsandbox外権限で実行します。helperが最初の外部確認前にtokenを消費するため再試行は1回に限られます。通常のfilesystem権限もこの実行では変わらないため、token不在や再失敗では権限昇格せず、直接`git merge`やcleanupへ迂回しません。
