...
1. `AGENTS.md`とPlan IDで識別できる承認済み計画を読む。正本は信頼済み投稿者による追跡Issue、プロジェクト指定docs、同一セッションのDaikiによる明示的合意の順で特定する。GitHub repositoryで追跡Issueがない場合は、計画承認にIssue作成が含まれるときだけ実装前に作成する。
...
互いに独立した事前調査、test結果の整理、非重複ファイルの実装だけをsubagentへ委譲する。仕様が固定された狭い実装は`gpt-5.6-luna`のxhigh、通常の実装は`gpt-5.6-terra`のmedium、複雑な実装は同modelのhighを使う。同じファイルを複数agentへ同時に編集させず、直列依存の単位はmain agentが処理する。main agentはsubagentの作業を重複せず、統合と受け入れ条件の確認に集中する。
...
1. `gpt-5.6-terra`のhigh: 正しさ、境界値、後方互換性、テスト不足
2. `gpt-5.6-sol`のhigh: セキュリティ、秘密情報、堅牢性、競合、リソース枯渇
3. `gpt-5.6-terra`のhigh: mergeへ反対する立場からの反論意見、計画・外部仕様・運用との不一致
