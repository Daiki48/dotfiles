...
3. 指定branchが既に選択されていれば一致を確認する。local既定branchへの復帰が必要なら、cleanなworktreeで`git switch <base>`を使う。新規作成が必要なら`git fetch origin <base>`後、`git switch -c <branch> origin/<base>`を使う。
