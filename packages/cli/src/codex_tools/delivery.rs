...
fn trusted_binary(path: &str, name: &str) -> Result<String> {
    ...
    let metadata =
        fs::symlink_metadata(path).map_err(|_| error(format!("system {name}を確認できません")))?;
    let parent = fs::symlink_metadata(path.parent().unwrap_or_else(|| Path::new("/")))
        .map_err(|_| error(format!("system {name}を確認できません")))?;
    #[cfg(unix)]
    {
        if !path.is_absolute()
