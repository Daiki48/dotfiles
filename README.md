...
| `codex` | Install Codex CLI via `npm install -g @openai/codex`. Symlinks shared instructions, safety rules, and Skills; release-builds and atomically installs the Rust Git/GitHub hook plus `codex-worktree` and `codex-delivery` multi-call helpers; then idempotently migrates `~/.codex/config.toml`. Machine-local trust and TUI settings are preserved. | no |
