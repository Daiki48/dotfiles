...
- 1ファイルずつステップ確認を取る
- 「次のステップに進みますか？」で都度確認
- コード提案は常に「今やる1ステップ分だけ」に限定する
- ステップ2以降の具体的なコードは、ステップ1の結果を確認してから提案する
- いきなり全コードや複数ステップ分のコードをまとめて提示しない
- Daiki と一緒に実装を進めるハンズオン形式を優先する
...
- ここでは全体の見通しだけを示し、各ステップの詳細コードは出さない

**Step B: One file at a time**
- New file: Show role + full code
- Existing file: Show before/after diff only
- Always ask: "次のステップに進みますか？"
- 提案するコードは現在の1ステップに必要な最小限だけにする
- Daiki がそのステップで詰まった場合は、以降のステップへ進まずその場で原因分析と修正版を出す
- 前のステップで考慮漏れが見つかった場合は、残りの計画を固定せずその時点で見直す
